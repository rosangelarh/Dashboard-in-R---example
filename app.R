...
setwd("C:\\Users\\KMontano\\Downloads\\1 Data")
all_data <- import("Audit_Form_MG_prep_1.xlsx")  
all_data2 <- import("Audit_Form_MG_prep_ot_1.xlsx")  
...
server <- function(input,output){
  ...
  output$table6 <- renderTable({
    ...
    table_data6_1_1 <- table_data6_1%>%group_by(AUDITOR)%>% reframe(NUM_P = first(NUM_M), DUR_P=first(DUR_M))
  ...
  output$table7 <- renderTable({
    ...
    table_data7_1_1 <- table_data7_1%>%group_by(ENCUESTADOR)%>% reframe(IND_P = first(IND_M), CONS_P=first(LENG_M), LENG_P=first(LENG_M), U_AD_P=first(U_AD_M), U_N_P=first(U_N_M), PREG_P=first(PREG_M) )
  ...
  output$table8 <- renderTable({
    ...
    table_data8 <- table_data8_1%>% reframe(NUM_P = first(NUM_M), DUR_P=first(DUR_M))
    table_data8
  })
